...
pub type SelectorCallback<R, I> = for<'a> fn(
    &'a mut R,
    I,
    ...
    I: SelectorItem + 'static,
{
    ...
    pub fn new_with_theme(
        ...
        assert!(!options.is_empty(), "RecorderSelect requires at least one option.");
            ...
            _task: task::spawn(async move {
                ...
                loop {
                    ...
                    if let Some(row_index) = touched_index {
                        if row_index < state.options.len() {
                    ...
                    let redraw_rows: Vec<(usize, String)> = dirty_rows
                        .into_iter()
                        .filter_map(|index| {
                            state.options.get(index).map(|item| (index, item.label().to_string()))
                        })
                        .collect();
